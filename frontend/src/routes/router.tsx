...
import { BranchManagerDashboard } from '@/features/dashboard/pages/BranchManagerDashboard';
import { BranchStaffPage, BranchAssignedCentersPage, BranchLabDetailsPage } from '@/features/branch/pages';
...
import { BranchListPage } from '@/features/company/branch/pages/BranchListPage';
import { CreateBranchPage } from '@/features/company/branch/pages/CreateBranchPage';
import { EditBranchPage } from '@/features/company/branch/pages/EditBranchPage';
import { BranchDetailsPage } from '@/features/company/branch/pages/BranchDetailsPage';
import { BranchSetupWizardPage } from '@/features/company/branch/pages/BranchSetupWizardPage';
import { BranchPendingVerificationsPage } from '@/features/company/branch/pages/BranchPendingVerificationsPage';
...
const RoleBasedDashboardRedirect = () => {
  ...
  if (role === 'PAPER_SETTER' || role === 'QUESTION_SETTER' || role === 'Paper Setter') return <Navigate to="/dashboard/paper-setter" replace />;
  if (role === 'OBSERVER' || role === 'Observer') return <Navigate to="/dashboard/observer" replace />;
  if (role === 'TECHNICAL_MANAGER' || role === 'Technical Manager') return <Navigate to="/dashboard/technical-team" replace />;
  if (role === 'CENTER_MANAGER' || role === 'Center Manager') return <Navigate to="/dashboard/center-manager" replace />;
  if (role === 'BRANCH_MANAGER' || role === 'Branch Manager') return <Navigate to="/dashboard/branch-manager" replace />;
  ...
  if (role === 'BIOMETRIC_VERIFIER' || role === 'Biometric Verifier') return <Navigate to="/dashboard/biometric-verifier" replace />;
  ...
  if (role === 'GOVT_AUTHORITY' || role === 'Govt Authority' || role === 'Government Authority') return <Navigate to="/dashboard/govt-authority" replace />;
...
export const router = createBrowserRouter([
  ...
  // Branch Manager Mandatory Onboarding Setup Wizard
  {
    path: '/branch/onboarding-wizard',
    element: <ProtectedRoute><BranchSetupWizardPage /></ProtectedRoute>,
  },
        ...
        children: [
          ...
          },
          {
            ...
            children: [
              ...
              { path: 'branch-manager/centers', element: <BranchAssignedCentersPage /> },
              { path: 'branch-manager/staff', element: <BranchStaffPage /> },
              { path: 'branch-manager/labs', element: <BranchLabDetailsPage /> },
          ...
          // ─── Company Admin Routes ────────────────────────
          {
            ...
            children: [
              ...
              { path: 'branches/pending-verifications', element: <BranchPendingVerificationsPage /> },
          ...
          // ─── Master Admin Routes ────────────────────────
          {
            ...
            children: [
              ...
              { path: 'access-management/users', element: <Navigate to="/master-admin/access-management?tab=users" replace /> },
              { path: 'access-management/roles', element: <Navigate to="/master-admin/access-management?tab=roles" replace /> },
