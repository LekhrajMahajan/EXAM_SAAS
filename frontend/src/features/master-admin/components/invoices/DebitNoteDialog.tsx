...
import { useGenerateDebitNote } from "../../hooks/invoice.hooks";
...
export const DebitNoteDialog: React.FC<DebitNoteDialogProps> = ({
  isOpen,
  onClose,
  invoice,
  availableCredit,
}) => {
  const { mutate: generateDebitNote, isPending } = useGenerateDebitNote();
  
  const form = useForm<DebitNoteFormValues>({
    resolver: zodResolver(debitNoteSchema),
    defaultValues: {
      amount: undefined as unknown as number, // Let user type the debit amount
      reason: "",
      remarks: "",
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <DialogTitle>Generate Debit Note</DialogTitle>
          <DialogDescription>
            Create a debit note linked to invoice {invoice.invoiceNumber}.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-lg mb-4 text-sm">
          ...
          <div>
            <span className="text-slate-500 block">Existing Credit Balance</span>
            <span className="font-medium text-emerald-600">${availableCredit.toFixed(2)}</span>
          </div>
        </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="e.g. Additional services rendered"
            />
