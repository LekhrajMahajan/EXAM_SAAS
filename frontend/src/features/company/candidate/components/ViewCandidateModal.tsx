...
import type { Center } from '@/features/company/center/types/center.types'
...
export const ViewCandidateModal = ({ isOpen, onClose, candidate }: ViewCandidateModalProps) => {
  const [liveCenter, setLiveCenter] = useState<Center | null>(null)
  const [isLoadingCenter, setIsLoadingCenter] = useState(false)

  useEffect(() => {
    let isMounted = true
    const fetchCenterDetails = async () => {
      if (!isOpen || !candidate?.centreCode) {
        setLiveCenter(null)
        return
      }
      ...
      try {
        const response = await centerApi.getAll({ search: candidate.centreCode })
        if (isMounted && response?.success && response.data?.data) {
          const match = response.data.data.find(
            (c: Center) => c.centerCode === candidate.centreCode,
          )
          setLiveCenter(match || null)
        } else if (isMounted && response?.success && response.data?.centers) {
          const match = response.data.centers.find(
            (c: Center) => c.centerCode === candidate.centreCode,
          )
          setLiveCenter(match || null)
        }
      } catch (error) {
        console.error('Failed to fetch live center details', error)
      } finally {
        if (isMounted) setIsLoadingCenter(false)
      }
    }
          ...
          <div className='grid gap-6 py-4 px-2'>
            ...
            <div>
              <div className='flex items-center justify-between border-b pb-2 mb-4'>
                ...
                {!isLoadingCenter && liveCenter && (
                  <div className='text-[10px] bg-green-50 text-green-700 px-2 py-0.5 rounded-full border border-green-200'>
                    Live Data
                  </div>
                )}
              </div>
              ...
              <div className='grid grid-cols-2 md:grid-cols-3 gap-4'>
                <div className='space-y-1'>
                  ...
                  <p className='font-medium'>
                    {liveCenter ? liveCenter.centerName : candidate.centerName || '-'}
                  </p>
                </div>
                <div className='space-y-1'>
                  <Label className='text-muted-foreground'>Centre Code</Label>
                  <p className='font-medium'>
                    {liveCenter ? liveCenter.centerCode : candidate.centreCode || '-'}
                  </p>
                </div>
                <div className='space-y-1'>
                  <Label className='text-muted-foreground'>City</Label>
                  <p className='font-medium'>
                    {liveCenter ? liveCenter.city : candidate.city || '-'}
                  </p>
                </div>
                <div className='space-y-1'>
                  <Label className='text-muted-foreground'>District</Label>
                  <p className='font-medium'>
                    {liveCenter ? liveCenter.city : candidate.district || '-'}
                  </p>
                </div>
                <div className='space-y-1'>
                  <Label className='text-muted-foreground'>State</Label>
                  <p className='font-medium'>
                    {liveCenter ? liveCenter.state : candidate.state || '-'}
                  </p>
                </div>
                <div className='space-y-1'>
                  <Label className='text-muted-foreground'>PIN</Label>
                  <p className='font-medium'>
                    {liveCenter ? liveCenter.pincode : candidate.pin || '-'}
                  </p>
                </div>
                <div className='space-y-1 md:col-span-3'>
                  <Label className='text-muted-foreground'>Full Address</Label>
                  <p className='font-medium'>
                    {liveCenter ? liveCenter.address : candidate.fullCentreAddress || '-'}
                  </p>
                </div>
                <div className='space-y-1'>
