...
export const CandidateListPage = () => {
  ...
  const uniqueExams = useMemo(() => {
    const exams = new Set(importedCandidates.map(c => c.examName).filter(Boolean));
    return Array.from(exams) as string[];
  }, [importedCandidates]);

  const uniqueShifts = useMemo(() => {
    const shifts = new Set(importedCandidates.map(c => c.shift).filter(Boolean));
    return Array.from(shifts) as string[];
  }, [importedCandidates]);
