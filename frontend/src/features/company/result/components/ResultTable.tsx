...
export function ResultTable({ results }: ResultTableProps) {
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'Generated':
      case 'EVALUATED': return <span className="px-2 py-1 rounded bg-blue-50 text-blue-700 text-xs font-medium border border-blue-200">Generated</span>;
      case 'Published':
  ...
  const groupedResults = useMemo(() => {
    const map = new Map<string, { examName: string; candidates: CandidateResult[] }>();
    for (const res of results) {
      const key = res.exam;
      if (!map.has(key)) {
        map.set(key, { examName: res.exam || 'Unknown Exam', candidates: [] });
      }
  ...
  return (
    <div className="bg-card rounded-lg border border-border shadow-sm">
      <Accordion type="multiple" className="w-full" defaultValue={groupedResults.map((_, i) => `item-${i}`)}>
                ...
                <table className="w-full text-sm text-left text-muted-foreground">
                  <thead className="text-xs text-foreground uppercase bg-muted/50 border-b border-border">
                    <tr>
                      ...
                      <th scope="col" className="px-6 py-4">Marks Obtained</th>
                  ...
                  <tbody>
                    {group.candidates.map((res) => (
                      <tr key={res.id} className="border-b last:border-0 border-border hover:bg-muted/50 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="font-bold text-foreground">
                            {typeof res.marksObtained === 'number' && res.marksObtained % 1 !== 0 ? res.marksObtained.toFixed(2) : res.marksObtained}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="font-medium text-muted-foreground">{typeof res.totalMarks === 'number' && res.totalMarks % 1 !== 0 ? res.totalMarks.toFixed(2) : res.totalMarks}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
