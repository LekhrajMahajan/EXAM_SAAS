  ...
  const loginMutation = useMutation({
    ...
    onSuccess: ({ loginRes, profileRes }) => {
      if (profileRes.role !== 'MASTER_ADMIN' && profileRes.role !== 'Master Admin') {
        setErrorMsg('Access denied. This login page is for Master Admins only.');
        authService.logout();
        return;
      }
    ...
    <div className="flex min-h-screen items-center justify-center bg-muted/40 px-4 py-12 sm:px-6 lg:px-8">
      <div className="w-full max-w-md space-y-8 bg-background p-8 rounded-xl shadow-lg border">
        <div className="text-center">
          ...
          <h2 className="text-3xl font-bold tracking-tight text-primary">Master Admin Sign In</h2>
          <p className="mt-2 text-sm text-muted-foreground">
            Secure access to the system administration panel
          </p>
        </div>
          ...
          <div className="space-y-4 rounded-md shadow-sm">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="master@examsaas.com"
              />
              {errors.email && (
