...
class CenterService extends BaseService<ICenter> {
  ...
  */
  async create(payload: Partial<ICenter>) {
    ...
    // Map frontend root fields to profileExtension
    if ((payload as any).googleMapUrl !== undefined || (payload as any).emergencyContact !== undefined) {
      payload.profileExtension = {
        ...(payload.profileExtension || {}),
        googleMapUrl: (payload as any).googleMapUrl || "",
        alternateContact: (payload as any).emergencyContact || ""
      };
    }
  ...
  */
  async submitForVerification(centerId: string, reqUser?: any) {
    ...
    const onboarding = await CenterOnboarding.findOneAndUpdate(
      { centerId: center._id },
      { 
        $set: { 
    ...
    // Sync documents to CompanyAdminRequest if center belongs to a company
    if (center.companyId) {
      ...
      const docsToSync = (onboarding?.documents || []).map((doc: any) => ({
        documentType: doc.documentType,
        isMandatory: doc.isMandatory,
        fileName: doc.fileName,
        fileUrl: doc.fileUrl,
        version: doc.version,
        status: doc.status === "REJECTED" ? "PENDING" : doc.status // reset rejected to pending upon re-upload
      }));

      if (adminRequest) {
        adminRequest.documents = docsToSync;
        adminRequest.status = CompanyAdminRequestStatus.UNDER_REVIEW;
        adminRequest.lastDocumentUploadAt = new Date();
        await adminRequest.save();
      } else {
        const User = require("../auth/user.model").default;
        const companyAdmin = await User.findOne({ companyId: center.companyId, role: "COMPANY_ADMIN" });
        if (companyAdmin) {
          await CompanyAdminRequest.create({
            ...
            status: CompanyAdminRequestStatus.UNDER_REVIEW,
  ...
  */
  async verifyCenterSetup(centerId: string, status: CenterSetupStatus, remarks?: string, reqUser?: any) {
    ...
    // Auto-create an approved CompanyAdminRequest for the creator company
    // so they are automatically connected to their own center.
    if (status === CenterSetupStatus.ACTIVE) {
      const CompanyAdminRequest = require("./companyAdminRequest.model").default;
      const { CompanyAdminRequestStatus } = require("./companyAdminRequest.model");
      ...
      if (!existingRequest) {
        ...
        await CompanyAdminRequest.create({
          companyId: center.companyId,
          companyAdminId: reqUser?.id || reqUser?.userId || center.companyId, 
          centerId: center._id,
          status: CompanyAdminRequestStatus.APPROVED,
          shiftRates,
          documents: onboardingRecord.documents || [],
          mouFileUrl: center.mouPdfUrl || "",
          mouFileName: center.mouFileName || "",
          approvedAt: new Date()
        });
      }
    }
  ...
  */
  ...
  |--------------------------------------------------------------------------
  */
  async getAll(query: any, populateFields?: string[]) {
    // If companyId is provided, we need to fetch centers directly owned by the company, 
    // OR centers that are connected to the company via APPROVED CompanyAdminRequest.
    if (query.companyId) {
      const CompanyAdminRequest = require("./companyAdminRequest.model").default;
      const approvedRequests = await CompanyAdminRequest.find({
        companyId: query.companyId,
        status: "APPROVED"
      }).select("centerId").lean();

      if (approvedRequests.length > 0) {
        const connectedCenterIds = approvedRequests.map((req: any) => req.centerId);

        query.extraQuery = {
          ...query.extraQuery,
          $or: [
            { companyId: query.companyId },
            { _id: { $in: connectedCenterIds } }
          ]
        };
        
        // Delete the direct companyId filter so it doesn't conflict with $or
        delete query.companyId;
      }
    }

    const result = await super.getAll(query, populateFields);
  ...
  */
  async update(id: string, payload: Partial<ICenter>) {
    ...
    if ((payload as any).googleMapUrl !== undefined || (payload as any).emergencyContact !== undefined) {
      ...
      if ((payload as any).googleMapUrl !== undefined) {
        payload.profileExtension.googleMapUrl = (payload as any).googleMapUrl;
      }
      if ((payload as any).emergencyContact !== undefined) {
        payload.profileExtension.alternateContact = (payload as any).emergencyContact;
      }
    }
    ...
    if ((!payload.commercialAgreement || payload.commercialAgreement.length === 0) && (payload as any).shiftRates && Array.isArray((payload as any).shiftRates)) {
      payload.commercialAgreement = (payload as any).shiftRates.map((s: any) => ({
        shiftName: s.name || s.shiftName || "Standard Shift",
        pricePerCandidate: Number(s.price || s.pricePerCandidate) || 250,
        candidateCapacity: payload.capacity || center.capacity || 100,
        maximumCapacity: payload.capacity || center.capacity || 100,
        specialNotes: `Slot timings: ${s.timings || "Standard Hours"}`
      }));
    }
  ...
  async statistics(companyId?: string) {
    ...
    if (companyId) {
      ...
      if (approvedRequests.length > 0) {
        ...
        extraQuery = { companyId };
      }
    }

    const totalCenters = await centerRepository.count(extraQuery);
    return {
      totalCenters,
    };
  }
