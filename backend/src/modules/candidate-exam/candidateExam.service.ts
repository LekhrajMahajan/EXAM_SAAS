...
class CandidateExamService {
  ...
  */
  ...
  async startSection(payload: any) {
    const { sessionId, examId, subjectName } = payload;
    if (!examId || !subjectName) {
      throw new ApiError(HTTP_STATUS.BAD_REQUEST, "examId and subjectName are required.");
    }

    // Fetch the exam to get the timeAllottedMinutes for this subject
    ...
    const timeAllottedMinutes = subjectConfig?.timeAllottedMinutes || 0;

    const now = new Date();
    // Find existing session, or create one
    let session = await ExamSession.findOne({ examId, candidateId: sessionId });
    if (!session) {
    ...
    let actualStartForSection = now;
    if (session.sectionTimings.length === 0 && exam.examDate && exam.startTime) {
      ...
      if (examStart.getTime() < now.getTime()) {
        actualStartForSection = examStart;
      }
    }

    // Only add if not already started
    const alreadyStarted = session.sectionTimings.some((s: any) => s.subjectName === subjectName);
    if (!alreadyStarted) {
      session.sectionTimings.push({
        subjectName,
        startedAt: actualStartForSection,
        lockedAt: null,
        isLocked: false,
        timeAllottedMinutes,
      } as any);
    ...
    const timing = session.sectionTimings.find((s: any) => s.subjectName === subjectName);
    return {
      sessionId,
      subjectName,
  ...
  /*
  |--------------------------------------------------------------------------
  | Candidate Lock Section
  |--------------------------------------------------------------------------
  */

  async lockSection(payload: any) {
    const { sessionId, examId, subjectName } = payload;
    const now = new Date();

    if (examId && sessionId && subjectName) {
      await ExamSession.findOneAndUpdate(
        { examId, candidateId: sessionId, "sectionTimings.subjectName": subjectName },
        {
          $set: {
            "sectionTimings.$.isLocked": true,
            "sectionTimings.$.lockedAt": now,
            lastUpdatedAt: now,
          },
        }
      );

      // Part-gating logic: auto-advance currentPartIndex if all subjects in current part are locked
      const session = await ExamSession.findOne({ examId, candidateId: sessionId }).lean();
      const Exam = mongoose.models.Exam || mongoose.model("Exam");
      const exam = await Exam.findById(examId).lean() as any;

      if (session && exam && exam.partWiseCutoffEnabled && session.partOrder && session.partOrder.length > 0) {
        const activePartName = session.partOrder[session.currentPartIndex];
        const activePart = (exam.parts || []).find((p: any) => p.partName === activePartName);
        
        if (activePart) {
          const subjectIdsInPart = activePart.subjectIds || [];
          // Map subjectIds to subject names
          const subjectNamesInPart = subjectIdsInPart.map((id: any) => {
            const subj = (exam.subjects || []).find((s: any) => String(s.subjectId || s._id) === String(id));
            return subj ? subj.name : null;
          }).filter(Boolean);

          // Check if all these subjects are locked
          const allLocked = subjectNamesInPart.every((name: string) => {
            const timing = (session.sectionTimings || []).find((t: any) => t.subjectName === name);
            return timing?.isLocked === true;
          });

          if (allLocked && subjectNamesInPart.length > 0) {
            await ExamSession.findOneAndUpdate(
              { examId, candidateId: sessionId },
              { $inc: { currentPartIndex: 1 } }
            );
          }
        }
      }
  ...
  /*
  |--------------------------------------------------------------------------
  | Set Part Order (Part-wise exam flow)
  |--------------------------------------------------------------------------
  */

  async setPartOrder(payload: any) {
    const { sessionId, examId, partOrder } = payload;
    if (!examId || !sessionId || !partOrder || !Array.isArray(partOrder)) {
      throw new ApiError(HTTP_STATUS.BAD_REQUEST, "examId, sessionId and partOrder[] are required.");
    }
    ...
    await ExamSession.findOneAndUpdate(
      { examId, candidateId: sessionId },
      {
        $set: { partOrder, currentPartIndex: 0, lastUpdatedAt: now },
        $setOnInsert: { examId, candidateId: sessionId, sessionId, sectionTimings: [], startedAt: now },
      },
      { upsert: true, returnDocument: 'after' }
    );

    return { sessionId, examId, partOrder, status: "PART_ORDER_SET" };
  }
  async login(payload: any) {
    ...
    if (exam.examDate && exam.startTime) {
      ...
      if (!isNaN(startHours) && !isNaN(startMinutes)) {
        ...
        const istDateString = new Date(exam.examDate).toLocaleDateString('en-CA', { timeZone: 'Asia/Kolkata' });
        const startIsoString = `${istDateString}T${String(startHours).padStart(2, '0')}:${String(startMinutes).padStart(2, '0')}:00+05:30`;
        const startDateObj = new Date(startIsoString);
        
        // Login window starts 15 minutes before exam start
        loginWindowStart = new Date(startDateObj.getTime() - 15 * 60000);
        
        // Prefer endTime from DB if set; fall back to startTime + duration
        if (exam.endTime) {
          const [endHours, endMinutes] = exam.endTime.split(':').map(Number);
          if (!isNaN(endHours) && !isNaN(endMinutes)) {
            const endIsoString = `${istDateString}T${String(endHours).padStart(2, '0')}:${String(endMinutes).padStart(2, '0')}:00+05:30`;
            examEnd = new Date(endIsoString);
          }
        }
        if (!examEnd) {
          examEnd = new Date(startDateObj.getTime() + (exam.duration || 120) * 60000);
        }
      }
    }
  ...
  */
  ...
  async getQuestions(query: any) {
    ...
    if (sessionId && examId) {
      const session = await ExamSession.findOne({ examId, candidateId: sessionId }).lean();
      if (session) {
        realSectionTimings = (session.sectionTimings || []).map((t: any) => ({
          subjectName: t.subjectName,
          startedAt: t.startedAt,
          lockedAt: t.lockedAt,
          isLocked: t.isLocked,
          timeAllottedMinutes: t.timeAllottedMinutes,
        }));
        partOrder = session.partOrder || [];
    ...
    return {
      ...
      sectionalTimeLimitEnabled: exam.sectionalTimeLimitEnabled || false,
      partWiseCutoffEnabled: exam.partWiseCutoffEnabled || false,
      parts: exam.parts || [],
      partOrder,
      currentPartIndex,
      sectionTimings: realSectionTimings,
  ...
  async submitExam(payload: any) {
    ...
    if (activePaperId) {
      ...
      detailedQuestions = paperQuestions.map((q: any) => {
        // CRITICAL FIX: q._id is PaperQuestion ID; q.questionId is the populated Master Question
        ...
        return {
          ...
          questionText: masterQ.question || q.questionText,
          options: masterQ.options || q.options,
          correctAnswer: masterQ.correctAnswer || masterQ.correctOption || q.correctAnswer || q.correctOption,
    ...
    if (payload.answers && payload.statuses) {
      try {
        await CandidateExamAnswer.create({
          ...
          examId: payload.examId,
          examName: exam?.examTitle || 'Practice Exam',
          shift: exam?.shift || null,
          shiftId: exam?.shiftId || null,
          parts: exam?.parts || [],
