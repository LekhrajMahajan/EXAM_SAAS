...
class DashboardService {
  ...
  async getOverview(filter: IDashboardFilter) {
    ...
    const pendingCompanies = await Company.countDocuments({ status: false, approvalStatus: "PENDING" });
    const suspendedCompanies = await Company.countDocuments({ status: false, approvalStatus: "APPROVED" });
    ...
    const [todaysRevenueResult, monthlyRevenueResult, totalRevenueResult] = await Promise.all([
      Payment.aggregate([
      ...
      mongoose.model("Subscription").aggregate([
        { $match: { status: { $in: ["ACTIVE", "EXPIRED", "SUSPENDED", "CANCELLED"] } } },
        {
          $lookup: {
            from: "plans",
            localField: "planId",
            ...
            estimatedPrice: {
              $switch: {
                branches: [
                  ...
                  { case: { $eq: ["$billingCycle", "QUARTERLY"] }, then: "$plan.pricing.quarterlyPrice" },
                  { case: { $eq: ["$billingCycle", "HALF_YEARLY"] }, then: "$plan.pricing.halfYearlyPrice" },
                  { case: { $eq: ["$billingCycle", "YEARLY"] }, then: "$plan.pricing.yearlyPrice" },
                ],
                default: 0,
              },
            },
      ...
      case "Center Manager": {
        ...
        for (const record of assignedExamsList) {
          ...
          if (!isEnded && exam) {
            ...
            const matchedCenterId = centerQuery.centerId || record.centers.find((c: any) => c.matchedCenterId)?.matchedCenterId;
            ...
            activeExamsList.push({
              ...
              shiftName: shiftId?.shiftName || 'N/A',
              startTime: shiftId?.startTime || exam.startTime,
              endTime: shiftId?.endTime || exam.endTime,
  ...
  async getDashboardCharts(
    ...
    const activeCount = await Company.countDocuments({ status: true });
    const pendingCount = await Company.countDocuments({ approvalStatus: "PENDING" });
    const suspendedCount = await Company.countDocuments({ status: false, approvalStatus: "APPROVED" });
    const inactiveCount = await Company.countDocuments({ status: false, approvalStatus: "REJECTED" });
    ...
    // Subscription Trend Logic
    const subscriptionTrendAgg = await SubscriptionModel.aggregate([
      {
        $match: {
          createdAt: {
            $gte: new Date(`${currentYear}-01-01`),
    ...
    const plans = await Plan.find({}, { _id: 1, planName: 1 }).lean();
    
    const subscriptionSeries = plans.map(plan => {
      let cumulativeSub = 0;
      const data = allMonths.map((_, index) => {
        const monthData = subscriptionTrendAgg.find(
          (m: any) => m._id.month === index + 1 && String(m._id.planId) === String(plan._id)
        );
        cumulativeSub += monthData?.count || 0;
        return cumulativeSub;
      });
      return { name: plan.planName, data };
    });

    if (subscriptionSeries.length === 0) {
      // Fallback if no plans exist
      subscriptionSeries.push({ name: "Plans", data: new Array(12).fill(0) });
    }
    ...
    return {
      ...
      companyStatusDistribution: {
        ...
        series: [{ name: "Status", data: [activeCount, pendingCount, suspendedCount, inactiveCount] }],
      },
    };
  }
