...
export const meritList = asyncHandler(
    async (req: Request, res: Response) => {
        ...
        const results = await Result.find(filter)
            .sort({ marksObtained: -1, percentage: -1 })
            .limit(limit)
            .populate('candidateId', 'firstName lastName candidateFullName fullName name applicationNumber applicationNo email phone')
            .populate('examId', 'examTitle examCode')
            .lean();

        // Ensure we only include approved/published results for merit list
        const approvedResults = results.filter(r => 
            r.resultStatus === 'APPROVED' || 
            r.resultStatus === 'PUBLISHED' ||
            r.passStatus === 'PASSED' // Fallback
        );
