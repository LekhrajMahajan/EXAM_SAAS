...
export const getAllImportCenterAssignExams = async (req: Request, res: Response) => {
  try {
    ...
    } else if (req.user && req.user.role === 'COMPANY_ADMIN' && (req.user as any).companyId) {
      // Company Admin sees only their own company's exams
      const ExamModel = mongoose.model('Exam');
      const companyExams = await ExamModel.find({ companyId: (req.user as any).companyId }).select('_id').lean();
      const companyExamIds = companyExams.map(e => e._id);
      
      if (query.examId) {
        // In case an examId was passed, verify it belongs to the company
        const requestedIdStr = query.examId.toString();
        if (!companyExamIds.some(id => id.toString() === requestedIdStr)) {
          return res.status(200).json({ success: true, data: [] });
        }
      } else {
        query.examId = { $in: companyExamIds };
      }
    }
...
export const getAssignedExamsForCenter = async (req: Request, res: Response) => {
  try {
    ...
    let filteredAssignedExams = assignedExams;
    
    // Data Privacy Filter:
    // If a Company Admin is viewing this center's exams, they should only see exams that their company created
    const user = (req as any).user;
    if (user && user.companyId && user.role === 'COMPANY_ADMIN') {
      filteredAssignedExams = assignedExams.filter((record: any) => {
        return record.examId && record.examId.companyId && record.examId.companyId.toString() === user.companyId.toString();
      });
    }

    const formattedExams = await Promise.all(filteredAssignedExams.map(async (record: any) => {
