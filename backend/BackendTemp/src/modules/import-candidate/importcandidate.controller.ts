...
export const getImportedCandidates = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if ((req as any).user && (req as any).user.role === 'PRIVATE_AUTHORITY') {
    ...
    } else if ((req as any).user && (req as any).user.role === 'COMPANY_ADMIN' && (req as any).user.companyId) {
      const ExamModel = mongoose.model('Exam');
      const companyExams = await ExamModel.find({ companyId: (req as any).user.companyId }).select('_id').lean();
      ...
      if (query.examId) {
        ...
        if (!companyExamIds.some(id => id.toString() === requestedIdStr)) {
          res.status(httpStatus.OK).json({ success: true, message: 'Imported candidates fetched successfully', data: [] });
          return;
        }
      } else {
