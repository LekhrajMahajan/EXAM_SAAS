...
class UserRepository extends BaseRepository<any> {
  ...
  async updateProfile(userId: string, payload: Partial<IUpdateProfile>) {
    const Model = await this.getModelForUser(userId);
    const updatedUser = await Model.findByIdAndUpdate(userId, payload, { returnDocument: 'after', runValidators: true }).select("-password");
    
    // Also update Employee collection if it exists (e.g. for Exam Managers)
    try {
      await Employee.findOneAndUpdate({ userId }, payload, { returnDocument: 'after', runValidators: true });
    } catch (err) {
  ...
  async updateProfileImage(userId: string, profileImage: string) {
    const Model = await this.getModelForUser(userId);
    return Model.findByIdAndUpdate(userId, { profileImage }, { returnDocument: 'after' }).select("-password");
  }
