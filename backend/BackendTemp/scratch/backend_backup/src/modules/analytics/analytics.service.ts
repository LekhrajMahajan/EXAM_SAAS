  ...
  */
  ...
  async getDashboardAnalytics(filter: IAnalyticsFilter) {
    const [
      ...
      this.getNotificationAnalytics(filter),
    ]);

    const unreadNotificationCount = notifications.find((n: any) => n._id === "UNREAD")?.count || 0;
    const orgScore = centers.totalCenters > 0 ? Math.round((centers.verifiedCenters / centers.totalCenters) * 100) : 100;

    return {
      period: filter.period ?? AnalyticsPeriod.TODAY,
      organizationHealth: {
        score: orgScore,
        status: system.status === "HEALTHY" && orgScore > 80 ? "OPTIMAL" : "WARNING",
        uptime: system.uptimeSeconds,
        systemHealth: system.status,
      },
      todaysOperations: {
        upcomingExams: exams.upcomingExams || 0,
        runningExams: exams.runningExams || 0,
        completedExams: exams.completedExams || 0,
        todayAttendancePercentage: attendance.presentPercentage || 0,
        staffUtilizationRate: assignments.assignmentSuccessRate || 0,
      },
      ...
      alertsAndNotifications: {
        pendingApprovals: (companies.pendingVerification || 0) + (centers.pendingCenters || 0),
        criticalAlertsCount: live.liveViolationsCount || 0,
        auditSummaryCount: (companies.pendingVerification || 0) + (centers.pendingCenters || 0) + (live.liveViolationsCount || 0),
