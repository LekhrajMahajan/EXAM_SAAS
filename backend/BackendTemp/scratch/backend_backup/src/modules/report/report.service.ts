    ...
    async getUsersList(query: any = {}) {
        ...
        const enriched = users.map((u: any) => {
            ...
            return {
                ...
                company: emp.companyId?.companyName || null,
                branch: emp.centerId?.name || null,
    ...
    async getUsersExport(query: any = {}) {
        ...
        return users.map((u: any) => {
            ...
            return {
                ...
                company: (emp.companyId as any)?.companyName || '',
                branch: (emp.centerId as any)?.centerName || '',
    ...
    */
    ...
    async getExamList(query: any) {
        ...
        candidates.forEach(c => {
            candidateMap[c._id.toString()] = c.count;
        });
        ...
        const data = exams.map((e: any) => {
            ...
            const candidatesAssigned = candidateMap[examId] || (e.candidateIds ? e.candidateIds.length : 0);
            ...
            return {
                ...
                branch: e.centerId?.centerName || "N/A",
                candidatesAssigned,
    ...
    */
    ...
    async getFinancialSummary(query: any) {
        const invoices = await Invoice.find({ isDeleted: false })
            ...
            .populate("companyId", "subscriptionPlan");
        ...
        invoices.forEach((inv) => {
            if (inv.status === InvoiceStatus.PAID) {
                const planName = (inv as any).subscriptionId?.planId?.planName || (inv as any).companyId?.subscriptionPlan || "Other";
    ...
    */
    ...
    async getSecurityExport(query: any = {}) {
        ...
        const rows = data.map((r: any) => [
            ...
            r.companyId ? r.companyId.companyName : "N/A",
            "N/A",
            r.ipAddress || "N/A",
            r.device || "N/A",
            r.browser || "N/A",
