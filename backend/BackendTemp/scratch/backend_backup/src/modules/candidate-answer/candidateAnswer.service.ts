...
class CandidateAnswerService extends BaseService<ICandidateAnswer> {
    ...
    */
    private async validateSubmission(
        ...
        if (questionId) {
            ...
            const pq = await PaperQuestion.findOne({ questionId: questionId }).lean();
            if (pq && pq.subjectId) {
                const sectionTiming = submission.sectionTimings?.find(
                    (s: any) => String(s.subjectId) === String(pq.subjectId)
                );
                if (sectionTiming?.isLocked) {
                    throw new ApiError(
                        HTTP_STATUS.FORBIDDEN,
                        "This section is locked. Time for this subject has expired."
                    );
                }
            }
        }
    ...
    */
    async update(
        ...
        if (answer) {
            await this.validateSubmission(answer.submissionId.toString(), answer.questionId.toString());
        }
